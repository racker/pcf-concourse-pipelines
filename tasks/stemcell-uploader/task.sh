...
if [[ $RUNTIME == "windows" ]]; then
    STEMCELL_NAME=light-bosh-stemcell-$SC_VERSION-aws-xen-hvm-windows2012R2-go_agent.tgz
else
    STEMCELL_NAME=light-bosh-stemcell-$SC_VERSION-aws-xen-hvm-ubuntu-trusty-go_agent.tgz
fi
